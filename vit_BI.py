...
def coatnet_2():
    ...
    return CoAtNet((224, 224), 4, num_blocks, channels, num_classes=1)
...
PATH = 'Mopticon/'
test_dataset = data_manage.CustomDataset(PATH + '22_21_test_BI.csv', PATH, mode='test')
train_dataset = data_manage.CustomDataset(PATH + '22_21_train_BI.csv', PATH, mode='train')
...
def train(epochs):
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            class_loss = criterion(torch.sigmoid(output), label)
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(testloader, 0):
                ...
                output_labels = np.zeros((output.shape[0], 1))
                for idx, output_ele in enumerate(output):
                    output_ele = torch.sigmoid(output_ele)
                    if output_ele > 0.5:
                        output_labels[idx] = 1
                    else:
                        output_labels[idx] = 0
                        
                gt = np.array(gt.cpu().detach().tolist())
                for idx, label in enumerate(gt):
                    tot += 1
                    if label == output_labels[idx] - 1:
                        cor += 1
            
            print('Epoch:{},Elapsed time:{:.2f}'.format(epoch, time.time() - stime))
        ...
        if epoch in [80, 100, 120, 140, 160, 180, 199]:
            torch.save(model.state_dict(),
                       '/home/MMI22jonguk/Mopticon/cp_BI_binary/checkpoint{}_acc:{}_cost:{}.pth'.format(epoch, acc,
                                                                                                         class_running_loss / batch))
        """
        if best_score <= acc:
            torch.save(model.state_dict(),
                       '/home/MMI22jonguk/Mopticon/cp_BI_binary/checkpoint{}_acc:{:.3f}.pth'.format(epoch, acc))
            best_score = acc
