...
def coatnet_2():
    ...
    return CoAtNet((224, 224), 4, num_blocks, channels, num_classes=4)
...
PATH = 'Mopticon/'
test_dataset = data_manage.CustomDataset(PATH + '22_21_test_300_noBI.csv', PATH, mode='test')
train_dataset = data_manage.CustomDataset(PATH + '22_21_trainset_300_noBI.csv', PATH, mode='train')
    ...
    for epoch in range(epochs):
        ...
        if epoch in [80, 100, 120, 140, 160, 180, 199]:
            torch.save(model.state_dict(),
                       '/home/MMI22jonguk/Mopticon/cp/checkpoint{}_acc:{}_cost:{}.pth'.format(epoch, acc,
                                                                                                         class_running_loss / batch))

        if best_score <= acc:
            torch.save(model.state_dict(),
                       '/home/MMI22jonguk/Mopticon/cp/checkpoint{}_acc:{}.pth'.format(epoch, acc))
            best_score = acc
